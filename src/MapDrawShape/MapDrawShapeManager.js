...
export default class MapDrawManager {
   ...
   */
  resetDrawnShape = () => {
    this.drawnShape?.forEach(p => p.setMap(null));
    this.deleteDrawnShape?.remove();
  ...
  #draw = () => {
    ...
    google.maps.event.addListenerOnce(this.map, "dblclick", () => {
      setTimeout(() => { this.#drawComplete(); }, 1);
    });
  }
  ...
  #drawComplete = () => {
    ...
    if (polygons.length > 0) {
      ...
      polygons.forEach((p) => {
        ...
        shape = shape.concat(p.map((item) => { return { lat: item.lat(), lng: item.lng() }; }));
      });
